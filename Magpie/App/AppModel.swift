...
final class AppModel: ObservableObject {
    ...
    /// Register APNs push token with the backend after permission is granted.
    private func registerPushTokenIfNeeded() async {
        guard let nm = notificationManager else { return }

        // Ensure notification permission is granted (shows system prompt if needed)
        if !nm.isPermissionGranted {
            let granted = await nm.requestPermission()
            guard granted else { return }
        }

        // Request remote notification registration (triggers AppDelegate callback)
        nm.registerForRemoteNotifications()

        // Wait briefly for the token to arrive from the OS
        for _ in 0..<10 {
            if nm.deviceToken != nil { break }
            try? await Task.sleep(for: .milliseconds(500))
        }

        guard let token = nm.deviceToken else { return }

        do {
            try await backend.registerPushToken(token)
            logger.info("Push token registered with backend")
        } catch {
            logger.error("Push token registration failed: \(error.localizedDescription, privacy: .public)")
        }
    }
