...
struct SoulConversationScreen: View {
    ...
    private var magpieAvatar: some View {
        Image("MagpieLogo")
            .resizable()
            .scaledToFill()
    ...
    private func reportMessage(_ message: SoulMessage) {
        ...
        let mailto = "mailto:support@trymagpie.xyz?subject=\(subject.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? "")&body=\(body.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? "")"
