...
async function advanceConversation(conversationId: string) {
  ...
  if (shouldEvaluate) {
    ...
    await upsertImpressionEdge(conversation.user_a_id, conversation.user_b_id, { ...evaluationA, score: scoreA }, baUnlockedForA, encounterCountA);
  ...
  // Phase-aware conversation end check (independent of evaluation interval)
  const messageLimit = getMessagesForEncounter(encounterCount);
